...
import com.tj.resolvedemo.util.ResolveXlsFastFileThread;
...
@RestController
@RequestMapping("/fast")
public class FastExcelReadController {
    private static Logger logger = LoggerFactory.getLogger(FastExcelReadController.class);
    ...
    public ResultBean startInsertEasyExcelThreadAll(Integer num, String fileName, Integer threadNum) {
        ...
        try {
            ...
            for (int i = 0; i < length; i++) {
                executorService.execute(new ResolveXlsFastFileThread(start*i, start*(i+1), path, ext));
            }
            if (mod != 0 && length != 1) {
                executorService.execute(new ResolveXlsFastFileThread(start * threadNum, num, path, ext));
            }
