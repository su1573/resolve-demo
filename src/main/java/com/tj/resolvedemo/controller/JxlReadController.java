...
import com.tj.resolvedemo.util.ResolveXlsFileThread;
...
@RestController
@RequestMapping("/jxl")
public class JxlReadController {
    private static Logger logger = LoggerFactory.getLogger(JxlReadController.class);

    @Value("${target}")
    private String target;

    /**
    * @Description: 多线程调用，解析excel
    * @param: "[num, fileName, threadNum]"
    * @Return: com.tj.resolvedemo.util.ResultBean
    * @Author: supenghui
    * @Date: 2021/3/23 18:54
    */
    @GetMapping("/startInsertThreadAll")
    public ResultBean startInsertJxlThreadAll(Integer num, String fileName, Integer threadNum) {
...
//            ThreadPoolExecutor executorService = InitThreadPoolServlet.getExecutor();
            ...
            for (int i = 0; i < length; i++) {
                executorService.execute(new ResolveXlsFileThread(start*i, start*(i+1), path, ext));
            }
            if (mod != 0 && length != 1) {
                executorService.execute(new ResolveXlsFileThread(start * threadNum, num, path, ext));
            }
