...
import com.tj.resolvedemo.util.ResolveXlsPoiFileThread;
...
@RestController
@RequestMapping("/poi")
public class PoiReadController {
    ...
    */
    ...
    public ResultBean startInsertPoiThreadAll(Integer num, String fileName, Integer threadNum) {
...
//            ThreadPoolExecutor executorService = InitThreadPoolServlet.getExecutor();
            ...
            for (int i = 0; i < length; i++) {
                executorService.execute(new ResolveXlsPoiFileThread(start*i, start*(i+1), path, ext));
            }
            if (mod != 0 && length != 1) {
                executorService.execute(new ResolveXlsPoiFileThread(start * threadNum, num, path, ext));
            }
