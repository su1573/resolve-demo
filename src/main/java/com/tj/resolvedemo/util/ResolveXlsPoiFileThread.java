 ...
 */
public class ResolveXlsPoiFileThread implements Runnable {
    protected Logger logger = LoggerFactory.getLogger(ResolveXlsPoiFileThread.class);
    private String path;
    private String ext;
    ...
    public ResolveXlsPoiFileThread() {
    }

    public ResolveXlsPoiFileThread(Integer start, Integer end, String path, String ext) {
        this.path = path;
        this.ext = ext;
        this.start = start;
        this.end = end;
...
//            logger.info("---------- 线程名称：" + Thread.currentThread().getName() + " 开始工作 ----------");
            resolveMDBFileThreadUtil.dealPoiXlsFile(start, end, path, ext);
