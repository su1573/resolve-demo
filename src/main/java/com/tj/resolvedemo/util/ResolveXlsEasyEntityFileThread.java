 ...
 */
public class ResolveXlsEasyEntityFileThread implements Runnable {
    protected Logger logger = LoggerFactory.getLogger(ResolveXlsEasyEntityFileThread.class);
    private String path;
    private String ext;
    ...
    public ResolveXlsEasyEntityFileThread() {
    }

    public ResolveXlsEasyEntityFileThread(Integer start, Integer end, String path, String ext) {
        this.path = path;
        this.ext = ext;
        this.start = start;
        this.end = end;
...
//            logger.info("---------- 线程名称：" + Thread.currentThread().getName() + " 开始工作 ----------");
            resolveMDBFileThreadUtil.dealEasyExcelXlsFileEntity(start, end, path, ext);
