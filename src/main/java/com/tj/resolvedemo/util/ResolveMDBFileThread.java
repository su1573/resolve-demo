 ...
 */
public class ResolveMDBFileThread implements Runnable {
    protected Logger logger = LoggerFactory.getLogger(ResolveMDBFileThread.class);
    private String path;
    private String ext;
    ...
    public ResolveMDBFileThread() {
    }

    public ResolveMDBFileThread(Integer start, Integer end, String path, String ext) {
        this.path = path;
        this.ext = ext;
        this.start = start;
        this.end = end;
...
//            logger.info("---------- 线程名称：" + Thread.currentThread().getName() + " 开始工作 ----------");
            resolveMDBFileThreadUtil.dealOrderToDriver(start, end, path, ext);
